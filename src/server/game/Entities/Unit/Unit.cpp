    ...
    switch (dummySpell->SpellFamilyName)
    {
        ...
        case SPELLFAMILY_PALADIN:
        {
            ...
            if (dummySpell->SpellIconID == 3030)
            {
                ...
                // Add remaining ticks to healing dome
                basepoints0 += victim->GetRemainingPeriodicAmount(GetGUID(), triggered_spell_id, SPELL_AURA_PERIODIC_HEAL);
                break;
            }
            switch (dummySpell->Id)
            ...
            case SPELLFAMILY_PALADIN:
            {
                switch (auraSpellInfo->Id)
                {
                    ...
                    case 67758: // Item - Coliseum 25 Heroic Caster Trinket
                    {
                        ...
                        if (cooldown && GetTypeId() == TYPEID_PLAYER && ToPlayer()->GetSpellHistory()->HasCooldown(stack_spell_id))
                            return false;

                        CastSpell(this, stack_spell_id, true, NULL, triggeredByAura);

                        if (cooldown && GetTypeId() == TYPEID_PLAYER)
                            ToPlayer()->GetSpellHistory()->AddCooldown(stack_spell_id, 0, std::chrono::microseconds(NULL) + std::chrono::microseconds(cooldown));
...
{
    m_CombatTimer = 0;
    if (!(ToPlayer() && ToPlayer()->GetPet() && ToPlayer()->GetPet()->IsInCombat()))
